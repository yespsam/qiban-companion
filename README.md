...
当前 `v0.2.13` 版本包含高清男女 3D 角色模型、男女各 3 套皮肤、分步造型生成面板、参数化动作节奏（温柔/元气/稳重）、云端/本地真实语音声线选择和移动/桌面自适应构图。动作系统已重做：待机、散步、奔跑、挥手、点头、回应、转身和说话都使用新的表演骨架，统一处理呼吸、重心、肩胯、膝脚、头颈视线、手腕细节和动作淡入淡出；walk/run 不再使用第一版模型原生动画，而是使用正面可见的全身重心步态。云端模型会直接加载 GLB，加载期间不会先显示旧内置人物，根入口和旧手机 demo 入口都会进入新版 3D 人物页。
...
```bash
./scripts/package-release.sh v0.2.13
```

产物会生成到：

```text
dist/qiban-companion-portable-v0.2.13.zip
```

压缩包只包含可发布源码、启动脚本、静态资源和说明文档，不包含 `.git`、虚拟环境、运行日志、历史对话数据、测试截图或旧备份。

别人使用时从 GitHub Releases 下载 `qiban-companion-portable-v0.2.13.zip`，解压后按系统双击启动即可。
