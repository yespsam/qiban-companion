...
当前 `v0.2.15` 版本包含高清男女 3D 角色模型、男女各 3 套皮肤、分步造型生成面板、参数化动作节奏（温柔/元气/稳重）、云端/本地真实语音声线选择和移动/桌面自适应构图。动作系统已重做：待机、散步、奔跑、挥手、点头、回应、转身和说话都使用新的表演骨架，统一处理呼吸、重心、肩胯、膝脚、头颈视线、手腕细节和动作淡入淡出；`v0.2.15` 会优先使用模型原生走/跑动画，加载前才用程序步态兜底，并给缺少手指骨骼的当前模型叠加五指手部覆盖层。`v0.2.14` 合入了优化包里的 WebP 纹理版 GLB 和离线解码器，模型资源体积约减半，手机端打开会更快。默认背景保持透明，只有手动加 `?scene=room` 或 `?scene=night` 才显示可选场景背景。云端模型会直接加载 GLB，加载期间不会先显示旧内置人物，根入口和旧手机 demo 入口都会进入新版 3D 人物页。
...
```text
dist/qiban-companion-portable-v0.2.15.zip
```

压缩包只包含可发布源码、启动脚本、静态资源和说明文档，不包含 `.git`、虚拟环境、运行日志、历史对话数据、测试截图或旧备份。

别人使用时从 GitHub Releases 下载 `qiban-companion-portable-v0.2.15.zip`，解压后按系统双击启动即可。
