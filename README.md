...
当前 `v0.2.19` 版本优化了手机端整体 UI 和交互：打开后直接进入新版 3D 人物，顶部场景切换更适合触控，底部聊天输入、语音状态和播放提示更清楚，语音生成完成后可直接点击状态按钮播放。云端 Netlify 入口提供轻量场景回复与同源语音合成；连接本地电脑或随身盒子 API 时，手机端会优先调用本地 `/api/chat` 与 `/api/voice/speak`，实现真正的语音输入、聊天回复和语音播放。默认背景保持透明，只有手动加 `?scene=room` 或 `?scene=night` 才显示可选场景背景。
...
```bash
./scripts/package-release.sh v0.2.19
```

产物会生成到：

```text
dist/qiban-companion-portable-v0.2.19.zip
```

压缩包只包含可发布源码、启动脚本、静态资源和说明文档，不包含 `.git`、虚拟环境、运行日志、历史对话数据、测试截图或旧备份。

别人使用时从 GitHub Releases 下载 `qiban-companion-portable-v0.2.19.zip`，解压后按系统双击启动即可。
