...
当前 `v0.2.8` 版本包含高清男女 3D 角色模型、男女各 3 套皮肤、分步造型生成面板、参数化动作节奏（温柔/元气/稳重）、云端/本地真实语音声线选择和移动/桌面自适应构图。角色动作已改为每帧干净骨骼基准叠加，待机、挥手、散步和奔跑不会再串姿势；云端模型加载也已改为直接加载 GLB，避免生产页长时间空白。根入口和旧手机 demo 入口都会进入新版 3D 人物页，默认显示更明显的樱糖皮肤和造型控制。
...
```bash
./scripts/package-release.sh v0.2.8
```

产物会生成到：

```text
dist/qiban-companion-portable-v0.2.8.zip
```

压缩包只包含可发布源码、启动脚本、静态资源和说明文档，不包含 `.git`、虚拟环境、运行日志、历史对话数据、测试截图或旧备份。

别人使用时从 GitHub Releases 下载 `qiban-companion-portable-v0.2.8.zip`，解压后按系统双击启动即可。
