...
把 `qiban-companion-portable-v0.2.9.zip` 解压到一个普通文件夹，例如桌面或文档目录。
...
`v0.2.9` 已内置高清男女二次元 3D 角色、男女各 3 套皮肤、温柔/元气/稳重动作节奏和真实语音声线选择，旧手机 demo 入口也会自动进入新版 3D 人物页。走路和奔跑会优先使用模型自带 GLB 骨骼动画，动作更连贯；待机、挥手、点头、回应和说话会使用干净骨骼基准，避免动作之间互相串姿势。云端和本地都会直接加载 GLB 模型与动画，加载失败时才回退到内置人物。第一次打开时模型文件较大，等待几秒加载完成即可。
