...
把 `qiban-companion-portable-v0.2.3.zip` 解压到一个普通文件夹，例如桌面或文档目录。
...
画面只保留人物和必要的角色/对话/声线控制。点击人物即可互动；打开对话和语音时会连接本地 API 出声，点“声线”可选择女声的随身份、萝莉、御姐、搞笑女，或男声的随身份、少年、大叔、搞笑男。关闭对话时只保留静默动作。运行时请保持启动窗口打开；关闭窗口或按提示停止后，语音、记忆和模型后端也会停止。

`v0.2.3` 已内置高清男女二次元 3D 角色、稳定动作和真实语音声线选择，第一次打开时模型文件较大，等待几秒加载完成即可。
