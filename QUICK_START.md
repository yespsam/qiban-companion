...
把 `qiban-companion-portable-v0.2.15.zip` 解压到一个普通文件夹，例如桌面或文档目录。
...
`v0.2.15` 已内置高清男女二次元 3D 角色、男女各 3 套皮肤、温柔/元气/稳重动作节奏和真实语音声线选择，旧手机 demo 入口也会自动进入新版 3D 人物页。动作系统已重做：待机、散步、奔跑、挥手、点头、回应、转身和说话都使用新的表演骨架，统一处理呼吸、重心、肩胯、膝脚、头颈视线、手腕细节和动作淡入淡出；走路/奔跑会优先使用模型原生动画，加载前才用程序步态兜底。当前模型没有手指骨骼，`v0.2.15` 会叠加五指手部覆盖层，避免视觉上只剩三根手指。`v0.2.14` 合入优化包里的 WebP 纹理版 GLB 和离线解码器，模型资源体积约减半，手机端打开会更快。默认背景保持透明，只有手动加 `?scene=room` 或 `?scene=night` 才显示可选场景背景。云端和本地都会直接加载 GLB 模型，加载期间不会先显示旧内置人物，加载失败时才回退到内置人物。
