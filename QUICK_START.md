...
把 `qiban-companion-portable-v0.2.13.zip` 解压到一个普通文件夹，例如桌面或文档目录。
...
`v0.2.13` 已内置高清男女二次元 3D 角色、男女各 3 套皮肤、温柔/元气/稳重动作节奏和真实语音声线选择，旧手机 demo 入口也会自动进入新版 3D 人物页。动作系统已重做：待机、散步、奔跑、挥手、点头、回应、转身和说话都使用新的表演骨架，统一处理呼吸、重心、肩胯、膝脚、头颈视线、手腕细节和动作淡入淡出；walk/run 不再使用第一版模型原生动画，而是使用正面可见的全身重心步态。云端和本地都会直接加载 GLB 模型，加载期间不会先显示旧内置人物，加载失败时才回退到内置人物。第一次打开时模型文件较大，等待几秒加载完成即可。
